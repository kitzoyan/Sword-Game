...
---------------------------------
  ,  /  .     previous / next STATE  (IDLE, JUMPING, LANDING, LIGHT, HEAVY,
                                       CHARGE, AERIAL, arts, PARRYING p1/p2/p3,
                                       BLOCK, DODGE, ...)
...
def _jumping_subs():
    """Two sub-frames for the airborne tuck: rising and falling. The JUMPING pose
    branch tilts the torso by the sign of vertical velocity, so we set velocity.y
    to surface both variants. (Editing is frozen, so the value sticks until exit.)"""
    subs = []
    for label, vy in (("rising", 6.0), ("falling", -6.0)):
        def fn(f, vy=vy):
            _reset_anim_vars(f)
            v = f.body.velocity
            f.body.velocity = Vec3(v.x, vy, v.z)
            f.state = State.JUMPING
        subs.append((label, fn))
    return subs
...
def _build_categories():
    return [
        ...
        _Category("JUMPING", _jumping_subs()),
        _Category("LANDING", _simple_subs(State.LANDING)),
        _Category("LIGHT", _light_subs()),
        _Category("HEAVY", _attack_subs(AttackType.HEAVY)),
        _Category("CHARGE", _attack_subs(AttackType.CHARGE)),
        _Category("AERIAL", _attack_subs(AttackType.AERIAL)),
...
class PoseEditor:
    ...
    def _try_match_current_state(self):
        ...
        name_by_state = {
            ...
            State.JUMPING: "JUMPING", State.LANDING: "LANDING",
